...
class RMSNorm(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return einsum((x / rms_a).to(in_dtype), self.gain.to(in_dtype), "... d_model, d_model -> ... d_model")


class SwiGLU(nn.Module):
    def __init__(self, d_ff, d_model, device: torch.device | None = None, dtype: torch.dtype | None = None):
        super().__init__()
...
class MultiHeadAttention(nn.Module):
    def __init__(
        ...
        diagonal_mask = torch.tril(torch.ones((max_seq_len, max_seq_len), device=device, dtype=torch.bool), diagonal=0)
    ...
    def forward(
        ...
        head_rep = lambda M: rearrange(M, "... seq (n_heads d_head) -> ... n_heads seq d_head", n_heads=self.num_heads)
        hQ, hK, hV = head_rep(Q), head_rep(K), head_rep(V) #introduced the head dimension
        ...
        if self.store_cache:
            self._cache = {
                "Q": Q.detach(), "K": K.detach(), "V": V.detach(),       # pre-head-split projections
                "hQ": hQ.detach(), "hK": hK.detach(), "hV": hV.detach(), # head-split (hQ/hK post-rope if rope is set)
...
class TransformerBlock(nn.Module):
    def __init__(
        ...
        self.attention = MultiHeadAttention(
            d_model=d_model, num_heads=num_heads, max_seq_len=max_seq_len, rope=self.rope, device=device, dtype=dtype
        )
